...
type Plan = ReturnType<typeof buildImplementationPlan>
type Files = ReturnType<typeof buildProposedFiles>
type PackageData = ReturnType<typeof buildPrPackage>

export function ApprovalPage() {
  const { approved, approve, resetApproval, ticket, isCustomTicket } = useDemoState()
  ...
  return (
    <div className="page-container">
      <PageHeader
        eyebrow="Human checkpoint · APV-0042"
        title={approved ? 'Simulated draft pull request created' : 'Review package ready for approval'}
        description={approved ? 'The demo recorded an explicit approval and generated a local draft PR artifact. No GitHub API call was made.' : 'Automated work stops here. Review the plan, proposed artifacts, validation evidence, risk notes, and generated summary before authorizing the simulated transition.'}
        action={approved ? <span className="decision-badge badge-allowed"><CheckCircle2 size={12} /> Approved</span> : <span className="decision-badge badge-approval"><AlertTriangle size={12} /> Human action required</span>}
      />
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_390px]">
      <div className="space-y-6">
        <section className="surface p-6">
          <SectionTitle icon={<ClipboardCheck size={17} className="text-electric" />} title="Plan and proposed scope" />
          <div className="grid gap-3 md:grid-cols-2">{plan.map((item, index) => <div key={item.title} className="flex gap-3 rounded-lg border border-line/70 bg-ink/40 p-4"><span className="font-mono text-[10px] text-electric">0{index + 1}</span><div><div className="text-xs font-semibold text-slate-200">{item.title}</div><p className="mt-1 text-[11px] leading-5 text-slate-500">{item.detail}</p></div></div>)}</div>
          <div className="mt-6 space-y-3">{files.map((file) => <div key={file.path} className="flex items-center justify-between gap-4 border-t border-line/50 pt-3"><div className="flex min-w-0 items-center gap-3"><FileCode2 size={14} className="shrink-0 text-slate-600" /><code className="truncate text-[10px] text-blue-200">{file.path}</code></div><span className="shrink-0 font-mono text-[9px] text-teal-400">{file.change}</span></div>)}</div>
        </section>
        <section className="surface p-6">
          <SectionTitle icon={<GitPullRequestDraft size={17} className="text-violet-300" />} title="Generated pull-request summary" />
          <h3 className="text-base font-semibold text-white">{packageData.title}</h3>
          <p className="mt-3 text-sm leading-6 text-slate-400">{packageData.summary}</p>
          <div className="mt-5 rounded-lg border border-line bg-ink/55 p-4"><div className="detail-label">Reviewer focus</div><ul className="mt-3 space-y-2 text-xs text-slate-400">{reviewerFocus.map((item) => <li key={item}>• {item}</li>)}</ul></div>
        </section>
      </div>
