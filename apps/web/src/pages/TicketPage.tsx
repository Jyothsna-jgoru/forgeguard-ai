...
import { buildImplementationPlan, buildRagContext, isReferenceTicket } from '../data/scenario'
...
export function TicketPage() {
  ...
  const isCatalogTicket = ticket.source === 'catalog'
  const riskClass = ticket.risk === 'Low' ? 'badge-allowed' : ticket.risk === 'High' ? 'badge-blocked' : 'badge-approval'
  const pageTitle = hasReferencePatch ? 'Idempotent payment retries' : isCustomTicket ? 'Visitor-created engineering ticket' : 'Ready-made engineering ticket'
  const scenarioSource = hasReferencePatch ? 'Seeded reference' : isCatalogTicket ? 'Ready-made demo catalog' : 'Local visitor input'
  const riskRationale = hasReferencePatch
    ? 'the payment processor call is financially sensitive, but the proposed work is scoped to validation and replay behavior. No production data, deployment, or provider integration is available to the demo.'
    : `${isCatalogTicket ? 'This ready-made scenario is' : 'The visitor classified this scenario as'} ${ticket.risk.toLowerCase()} risk. ForgeGuard treats all ticket text as untrusted input and requires approval before the simulated external action.`

  return (
    <div className="page-container">
      <PageHeader eyebrow={`Ticket intake · ${ticket.key}`} title={pageTitle} description="The normalized work item, implementation intent, and retrieved engineering context supplied to the agent harness." action={<div className="flex flex-wrap gap-2"><span className={`decision-badge ${riskClass}`}><AlertTriangle size={12} /> {ticket.risk} risk</span><Link to="/intake" className="button button-secondary">Choose another ticket</Link></div>} />
      <div className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
        <div className="space-y-6">
          ...
          <RiskBanner><span><strong className="font-semibold text-amber-100">Risk rationale:</strong> {riskRationale}</span></RiskBanner>
        </div>

        <aside className="surface h-fit p-6"><SectionTitle eyebrow="Normalized context" title="Impact assessment" /><dl className="space-y-4 text-sm">{[['Primary service', ticket.service], ['Scenario source', scenarioSource], ['Change type', hasReferencePatch ? 'Behavior + tests' : 'Structured proposal'], ['Acceptance checks', String(ticket.acceptance.length)], ['External dependency', 'None connected'], ['Review gate', 'Required']].map(([label, value]) => <div key={label} className="flex justify-between gap-4 border-b border-line/50 pb-3 last:border-0"><dt className="text-slate-500">{label}</dt><dd className="text-right font-medium text-slate-200">{value}</dd></div>)}</dl></aside>
      </div>
