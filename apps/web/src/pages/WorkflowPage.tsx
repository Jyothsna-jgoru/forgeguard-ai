...
import { buildAgentSteps, isReferenceTicket } from '../data/scenario'
import { useDemoState } from '../state/useDemoState'

export function WorkflowPage() {
  const { approved, resetApproval, ticket, isCustomTicket } = useDemoState()
  const steps = useMemo(() => buildAgentSteps(ticket), [ticket])
  ...
  useEffect(() => {
    if (!playing || visibleCount >= steps.length) return
    const timer = window.setTimeout(() => {
      const nextCount = Math.min(visibleCount + 1, steps.length)
      setVisibleCount(nextCount)
      setSelected(nextCount - 1)
      if (nextCount >= steps.length) setPlaying(false)
    }, 900)
    return () => window.clearTimeout(timer)
  }, [playing, steps.length, visibleCount])

  const step = steps[selected]
  const completed = useMemo(
    () => Math.min(visibleCount, steps.length - (approved ? 0 : 1)),
    [approved, steps.length, visibleCount],
  )
  const restartTimeline = (autoPlay: boolean) => {
    resetApproval()
    setVisibleCount(1)
    setSelected(0)
    setPlaying(autoPlay)
  ...
  const replay = () => restartTimeline(false)
  const next = () => { const nextIndex = Math.min(visibleCount, steps.length - 1); setVisibleCount(Math.min(visibleCount + 1, steps.length)); setSelected(nextIndex) }
  const togglePlayback = () => {
    if (visibleCount >= steps.length) restartTimeline(true)
    else setPlaying((value) => !value)
  }

  return (
    <div className="page-container">
      <PageHeader eyebrow={isCustomTicket ? 'Custom ticket · Deterministic demo run' : 'Demo Scenario · Completed run'} title="Multi-agent workflow explorer" description={`Replay how ${ticket.key} moved through planning, repository analysis, change generation, validation, policy review, documentation, and a human checkpoint.`} action={<div className="flex gap-2"><button type="button" onClick={replay} className="button button-secondary"><RotateCcw size={14} /> Replay</button><button type="button" onClick={togglePlayback} className="button button-primary">{playing ? <Pause size={14} /> : <Play size={14} />}{playing ? 'Pause' : visibleCount >= steps.length ? 'Play again' : visibleCount === 1 ? 'Start replay' : 'Continue'}</button></div>} />

      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4"><Metric label="Ticket" value={ticket.key} /><Metric label="Agent stages" value="6 + gate" /><Metric label="Validation" value={hasReferencePatch ? '12 passed' : `${ticket.acceptance.length} mapped`} tone="teal" /><Metric label="External action" value={approved ? 'Draft PR approved' : 'Awaiting approval'} tone={approved ? 'teal' : 'amber'} /></div>

      <section className="surface mb-6 overflow-hidden">
        <div className="flex flex-col justify-between gap-4 border-b border-line/70 p-5 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><span className="rounded-md bg-electric/10 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-300">{ticket.key}</span><span className="text-[11px] text-slate-500">{ticket.service}</span></div><h2 className="mt-3 max-w-4xl text-base font-medium leading-6 text-white">{ticket.title}</h2></div><Link to="/ticket" className="inline-flex shrink-0 items-center gap-1 text-xs font-semibold text-slate-400 hover:text-white">View ticket <ChevronRight size={13} /></Link></div>
        <div className="h-1 bg-slate-800"><div className="h-full bg-gradient-to-r from-electric via-violet to-teal-400 transition-all duration-500" style={{ width: `${(visibleCount / steps.length) * 100}%` }} /></div>
      </section>

      <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <section ref={timelineRef} className="surface scroll-mt-24 p-4" aria-label="Agent workflow timeline">
          ...
          {visibleCount < steps.length && <button type="button" onClick={next} className="button button-secondary mt-4 w-full"><StepForward size={14} /> Advance one stage</button>}
        </section>
        ...
        <section className="surface overflow-hidden" key={step.id}>
          ...
          <div className="grid gap-px bg-line/60 lg:grid-cols-2">
            ...
            <div className="bg-[#0a1525] p-6"><div className="detail-label">Tool request</div><div className="mt-3 rounded-lg border border-line bg-ink/70 p-4"><code className="text-sm font-medium text-blue-200">{step.tool}</code><div className="mt-3 border-t border-line/60 pt-3 font-mono text-[10px] leading-5 text-slate-500">agent_role: {step.role}<br />environment: demo<br />scope: {hasReferencePatch ? 'samples/payment-service' : `${isCustomTicket ? 'local' : 'seeded'}-demo/${ticket.service}`}</div></div><div className="mt-7 flex items-center justify-between"><div className="detail-label">Gateway decision</div>{step.status === 'approval' && approved ? <span className="decision-badge badge-allowed"><Check size={12} /> Approval satisfied</span> : <DecisionBadge decision={step.decision} />}</div><div className="mt-3 flex gap-3 rounded-lg border border-line/70 bg-white/[0.02] p-4"><ShieldCheck className="mt-0.5 shrink-0 text-teal-300" size={17} /><p className="text-sm leading-6 text-slate-400">{step.status === 'approval' && approved ? 'Human approval APR-2026-0042 satisfied the gateway requirement. The draft PR remains simulated and no external mutation was performed.' : step.decisionReason}</p></div></div>
          </div>
        </section>
      </div>
    </div>
